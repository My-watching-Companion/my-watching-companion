...
router.post("/register/ok", async (req, res) => {
  ...
  try {
    ...
    (VerifLogin[0] !== undefined){
      ...
      await executeQuery(
        `INSERT INTO Users VALUES(GETDATE(), GETDATE(), '${Username}', '${LastName}', '${BirthDate}','${Mail}', 'Default', '${CryptoJS.AES.encrypt(Password,CRYPTO_KEY)}', '${FirstName}', 0, 0, 0, NULL, NULL)`      );
...
router.get("/users/:u/watchlists/:w", async (req, res) => {
  ...
  try {
    const query = await executeQuery(`SELECT U.UserID, U.ListsID, U.ListsName, U.UpdatedDate, A.ArtworkID, A.ArtworkName, RN.NatureLabel, RT.TypeName 
                                      from Users U 
...
router.get('/friends/:user', async (req,res)=>{
  const user = req.params['user']
  try{
    const friendslist = await executeQuery(`SELECT RF.FriendsUserID, U.Username, U.FirstName, U.LastName, U.UserProfilePicture, U.Confidentiality from Friend RF
